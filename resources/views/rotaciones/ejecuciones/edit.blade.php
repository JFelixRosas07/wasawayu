...
@section('title', 'Editar Ejecución Real')

@section('content_header')
<div class="d-flex justify-content-between align-items-center flex-wrap">
    <h1 class="mb-2">
        <i class="fas fa-pen text-success me-2"></i>
        Editar Ejecución Real
    </h1>
    <a href="{{ route('planes.show', $ejecucion->detalle->plan->id) }}" class="btn btn-outline-secondary">
        <i class="fas fa-arrow-left"></i> Volver al Plan
    </a>
</div>
<p class="text-muted">
    Plan: <strong>{{ $ejecucion->detalle->plan->nombre }}</strong> | 
    Año {{ $ejecucion->detalle->anio }} | 
    {{ $ejecucion->detalle->es_descanso ? 'Descanso' : ($ejecucion->detalle->cultivo->nombre ?? 'Sin cultivo planificado') }}
</p>
@stop
    ...
    <div class="info-card p-3 shadow-sm" style="border-radius: 16px; background: rgba(255,255,255,0.25);">
        ...
        <ul class="list-group list-group-flush mb-3">
            ...
            <li class="list-group-item bg-transparent">
                ...
                @if($ejecucion->detalle->es_descanso)
                    <span class="badge bg-secondary">Descanso</span>
                @else
                    {{ $ejecucion->detalle->cultivo->nombre ?? 'No definido' }}
                @endif
            </li>
            ...
            <li class="list-group-item bg-transparent">
                ...
                {{ $ejecucion->detalle->plan->parcela->nombre ?? 'No especificada' }}
            </li>

            <!-- Agricultor responsable -->
            <li class="list-group-item bg-transparent">
                <strong>Agricultor:</strong> 
                {{ $ejecucion->detalle->plan->parcela->agricultor->name ?? 'No asignado' }}
            </li>
            ...
            <li class="list-group-item bg-transparent">
                <strong>Inicio:</strong> {{ optional($ejecucion->detalle->fecha_inicio)->format('d/m/Y') ?? 'No definido' }}
            </li>
            <li class="list-group-item bg-transparent">
                <strong>Fin:</strong> {{ optional($ejecucion->detalle->fecha_fin)->format('d/m/Y') ?? 'No definido' }}
            </li>

            <!-- Estado -->
            <li class="list-group-item bg-transparent">
                <strong>Estado:</strong> 
                <span class="badge bg-info text-dark">{{ ucfirst($ejecucion->estado ?? 'pendiente') }}</span>
            </li>
        </ul>

        <!-- Imagen del cultivo planificado -->
        @if(!$ejecucion->detalle->es_descanso && $ejecucion->detalle->cultivo && $ejecucion->detalle->cultivo->imagen)
            <div class="text-center mt-3">
                <img src="{{ asset($ejecucion->detalle->cultivo->imagen) }}" 
                     alt="{{ $ejecucion->detalle->cultivo->nombre }}" 
                     class="img-fluid rounded shadow-sm"
                     style="max-height: 215px; object-fit: cover; width: 100%; border-radius: 14px;">
                <p class="mt-3 text-muted small">
                    {{ $ejecucion->detalle->cultivo->nombre }} – {{ ucfirst($ejecucion->detalle->cultivo->categoria) }}
                </p>
            </div>
        @endif
    </div>
        ...
        <div class="col-md-7">
            <div class="p-4 shadow-lg" style="border-radius: 20px; background: rgba(255,255,255,0.35);">
                ...
                <form action="{{ route('ejecuciones.update', $ejecucion->id) }}" method="POST" id="ejecucionForm">
                    @csrf
                    @method('PUT')

                    <!-- Cultivo real -->
                    @php
                        $cultivoSeleccionado = old('cultivo_real_id', $ejecucion->cultivo_real_id ?? $ejecucion->detalle->cultivo_id);
                    @endphp

                    <div class="form-group mb-3">
                        <label for="cultivo_real_id"><strong>Cultivo Real *</strong></label>
                        <select name="cultivo_real_id" id="cultivo_real_id"
                                class="form-select {{ !$ejecucion->cultivo_real_id ? 'border-warning bg-light' : '' }}" required>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="fecha_siembra" class="form-control"
                                   value="{{ old('fecha_siembra', optional($ejecucion->fecha_siembra)->format('Y-m-d')) }}" required>
                        </div>
                        <div class="col-md-6 mb-3">
                            <label><strong>Fecha real de cosecha</strong></label>
                            <input type="date" name="fecha_cosecha" class="form-control"
                                   value="{{ old('fecha_cosecha', optional($ejecucion->fecha_cosecha)->format('Y-m-d')) }}">
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" step="0.01" name="cantidad_sembrada" id="cantidad_sembrada"
                                   class="form-control" value="{{ old('cantidad_sembrada', $ejecucion->cantidad_sembrada) }}">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label><strong>Cant. cosechada</strong></label>
                            <input type="number" step="0.01" name="cantidad_cosechada" id="cantidad_cosechada"
                                   class="form-control" value="{{ old('cantidad_cosechada', $ejecucion->cantidad_cosechada) }}">
                        </div>
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="unidad_medida" id="unidad_medida" class="form-select">
                                @foreach(['cargas','arrobas','kg','qq','libras'] as $unidad)
                                    <option value="{{ $unidad }}" 
                                        {{ old('unidad_medida', $ejecucion->unidad_medida) == $unidad ? 'selected' : '' }}>
                                        {{ ucfirst($unidad) }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" step="0.01" name="area_cultivada" id="area_cultivada"
                                   class="form-control" value="{{ old('area_cultivada', $ejecucion->area_cultivada) }}">
                        </div>
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" name="rendimiento_produccion" id="rendimiento_produccion"
                                   ...
                                   value="{{ old('rendimiento_produccion', $ejecucion->rendimiento_produccion ? $ejecucion->rendimiento_produccion.' %' : '') }}">
                        </div>
                        <div class="col-md-4 mb-3">
                            <label><strong>Rendimiento de Parcela (por ha)</strong></label>
                            <input type="text" name="rendimiento_parcela" id="rendimiento_parcela"
                                   class="form-control bg-light" readonly 
                                   value="{{ old('rendimiento_parcela', $ejecucion->rendimiento_parcela ? $ejecucion->rendimiento_parcela.' por ha' : '') }}">
                        </div>
                    </div>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <textarea name="observaciones" rows="3" class="form-control"
                                  placeholder="Condiciones climáticas, plagas, rendimiento, etc.">{{ old('observaciones', $ejecucion->observaciones) }}</textarea>
                    </div>

                    <!-- Botones -->
                    <div class="text-end">
                        <button type="submit" class="btn btn-success">
                            <i class="fas fa-save"></i> Actualizar Ejecución
                        </button>
                        <a href="{{ route('planes.show', $ejecucion->detalle->plan->id) }}" class="btn btn-secondary">Cancelar</a>
                    </div>
                </form>
            </div>
        </div>
